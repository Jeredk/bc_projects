...
router.get('/', function (req, res, next) {
  ...
  res.render('index', { title: 'Express점심 11:50', name:req.session.name });
});
...
// get 방식, post 방식 등 method 방식과 혼용해서 사용 가능
// ~/url/:변수명
router.get('/info/:no', (req, res, next) => {
  ...
  // // no를 넣어서 해당 상품 저보를 가져와서 화면에 뿌린다.( render )
  // // 함수명 : selectItemByNo
  // // rows 데이터가 존재하면
  // res.render( 'info', { row:rows[0] } )
  // // rows 데이터 가 없다면
  // res.end( 'no items' )
})
