...
let target_site = 'http://nonojapan.com'

// 접속
request( target_site, (err,response,body)=>{
    if(!err){
        // 사이트가 응답하는 순수html 텍스트를 획득 ->body
        console.log(typeof body)
        // 1. body의 내용을 DOM으로 생성한다. -> 형식은 jQuery방식
        let $ = cheerio.load(body)
        console.log(typeof $)
        // 2. 추출 -> css selector를 통해서 대상을 특정하고, 추출한다
        // 2-1. 카테고리만 뽑아보지 : 특정한 내용은, class가 mdl-navigation__link인 요소
        ...
        // 상품 목록 특정 : mdl-cell mdl-card mld-shadow--4dp portfolio-card
